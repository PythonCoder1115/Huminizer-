...
{Fore.CYAN}                                             Discord Account Humanizer
{Fore.LIGHTBLACK_EX}                                                developed by Flame Tools{Style.RESET_ALL}
"""
    print(banner)
...
def main():
    ...
    try:
        threads_input = input(f"{Fore.MAGENTA}[{Fore.WHITE}?{Fore.MAGENTA}]{Style.RESET_ALL} How many threads do you want to use? (Default: {MAX_THREADS}): ").strip()
        if threads_input:
            MAX_THREADS = int(threads_input)
    except ValueError:
        log_simple("WARN", f"Invalid thread count entered. Using default: {MAX_THREADS}")
        ...
        while True:
            while len(active_futures) < MAX_THREADS:
                try:
                    ...
                    future = executor.submit(process_token, token, names, bios, pronouns_list, avatar_files, lazy_get_avatar)
                    future._token = token
    ...
    print(f"{Fore.WHITE}  Total Accounts      : {Fore.CYAN}{stats['total']}{Style.RESET_ALL}")
    print(f"{Fore.WHITE}  Success             : {Fore.GREEN}{stats['success']}{Style.RESET_ALL}")
    print(f"{Fore.WHITE}  Failed              : {Fore.RED}{stats['failed']}{Style.RESET_ALL}")
    print(f"{Fore.WHITE}  Success Rate        : {Fore.YELLOW}{success_rate:.2f}%{Style.RESET_ALL}")
    print(f"{Fore.WHITE}  Time Taken          : {Fore.LIGHTBLACK_EX}{format_time(elapsed)}{Style.RESET_ALL}")
    if proxy_manager:
        ps = proxy_manager.get_stats()
        print(f"{Fore.WHITE}  Proxies (Good/Bad)  : {Fore.GREEN}{ps['active']}{Style.RESET_ALL}/{Fore.RED}{ps['bad']}{Style.RESET_ALL}")
    print(f"{Fore.MAGENTA}═══════════════════════════════════════════{Style.RESET_ALL}")
